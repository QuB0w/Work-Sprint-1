...
|--------|----------|-------------|-------------|
| POST | `/auth/register` | Регистрация нового пользователя (204) | — |
| POST | `/auth/login` | Вход, получение JWT-токена (200) | — |
...
| POST | `/events` | Создание нового мероприятия | Admin |
| PUT | `/events/{id}` | Обновление существующего мероприятия | Admin |
...
| Метод | Эндпоинт | Описание | Авторизация |
|--------|----------|-------------|-------------|
| POST | `/events/{id}/book` | Создание брони на мероприятие (202 Accepted) | User/Admin |
| GET | `/bookings/{id}` | Получение текущего статуса брони | User/Admin |
| DELETE | `/bookings/{id}` | Отмена брони (204 No Content) | Владелец / Admin |
...
- `400 Bad Request`: Ошибки валидации или неверные параметры (в т.ч. бронирование прошедшего события)
- `401 Unauthorized`: Отсутствует или невалидный JWT-токен
- `403 Forbidden`: Недостаточно прав (например, отмена чужой брони без роли Admin)
- `404 Not Found`: Ресурс не найден
- `409 Conflict`: Нет свободных мест или превышен лимит активных бронирований
...
## Новые возможности в Sprint 8

### ✅ JWT-аутентификация и авторизация

Добавлена сущность `User` с полями: идентификатор, логин, хеш пароля и роль (`Admin` / `User`).

**Эндпоинты аутентификации** (доступны без токена):
- `POST /auth/register` — регистрация нового пользователя (роль `User` по умолчанию, можно передать `Admin`). Возвращает 204 No Content.
- `POST /auth/login` — вход по логину и паролю, возвращает JWT-токен (200 OK). При неверных учётных данных — 404.

**Защищённые эндпоинты:**
- `POST /events/{id}/book`, `GET /bookings/{id}`, `DELETE /bookings/{id}` — требуют аутентификации (401 без токена).
- `POST /events/`, `PUT /events/{id}`, `DELETE /events/{id}` — доступны только роли `Admin` (403 для обычного пользователя).

### ✅ Ролевая модель

| Роль | Управление событиями | Бронирование | Отмена бронирования |
|------|---------------------|-------------|---------------------|
| **Admin** | Создание, редактирование, удаление | Да | Любые брони |
| **User** | Только просмотр | Да | Только свои брони |

### ✅ Доменные правила бронирования

- **Запрет бронирования прошедшего события** — возвращает 400, если событие уже началось.
- **Лимит активных броней** — не более 10 активных (Pending/Confirmed) бронирований на пользователя. При превышении — 409 с указанием лимита.
- **Лимиты разных пользователей независимы** — лимит одного пользователя не влияет на другого.
- **Отмена брони** (`DELETE /bookings/{id}`) — пользователь может отменить только свою бронь, администратор — любую. При нарушении — 403.
- Добавлен статус `Cancelled` в перечисление `BookingStatus`.

### ✅ Безопасность

- Пароли хранятся в виде SHA-256 хеша, не открытым текстом.
- JWT-секрет, издатель, аудитория и время жизни токена вынесены в `appsettings.json` (секция `Jwt`).
- **Рекомендация:** в продакшне замените значение `Jwt:Secret` на безопасное, например через переменные окружения или Secret Manager.

### Получение JWT-токена через Swagger

1. Откройте Swagger UI (`/swagger`).
2. Выполните `POST /auth/register` с телом: `{"login": "admin", "password": "admin123", "role": "Admin"}`.
3. Выполните `POST /auth/login` с телом: `{"login": "admin", "password": "admin123"}`.
4. Скопируйте значение поля `token` из ответа.
5. Нажмите кнопку **Authorize** в Swagger UI и вставьте токен.
6. Все последующие запросы будут отправляться с заголовком `Authorization: Bearer <token>`.
...
{
  "Jwt": {
    "Secret": "ThisIsASuperSecretKeyForJwtThatMustBeAtLeast32Chars!",
    "Issuer": "EventApi",
    "Audience": "EventApiClients",
    "ExpiresInMinutes": 60
  }
}
```
